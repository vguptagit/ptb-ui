...
const Matching = props => {
  ...
  const getEditView = () => {
    ...
    const areOptionsFilled = formData.Options.every(
      opt => opt.option.trim() !== '' && opt.matchingOption.trim() !== ''
    );
        ...
        <Form className="editmode border rounded p-3 bg-light">
          <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
            <Form.Label className="mb-1" style={{ fontSize: '0.9em' }}>
              {props.questionNode.qtiModel.QstnSectionTitle}
            </Form.Label>
            <Form.Control
              name="Caption"
                      ...
                      <Form.Group style={{ marginBottom: '5px', width: '50%' }}>
                        <div>
                          <Form.Control
                            ...
                            type="text"
                            placeholder={
                              props.questionNode.qtiModel.editOption_Column_A1 +
                              (index + 1) +
                              props.questionNode.qtiModel.editOption_Column_A2
                            }
                            maxLength={255}
                          />
                        </div>
          ...
          <div className="w-100 ml-1">
            <div className="mr-2">
              ...
              <span
                ...
                dangerouslySetInnerHTML={sanitizedData(questionNode.qtiModel.Caption)}
              ></span>
            </div>
          </div>
              ...
              <div key={index} className="view-question">
                <div className="text-section d-flex flex-wrap">
                  <span
                    ...
                    style={{ minWidth: '250px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                    title={option.option}
                    dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(`A${index + 1}) ${option.option}`) }}
                  ></span>
                  <span
                    className="ml-3 ml-md-0"
                    style={{ minWidth: '100px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                    title={option.matchingOption}
                    dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(`B${index + 1}) ${option.matchingOption}`) }}
                  ></span>
                </div>
              </div>
            ))}
