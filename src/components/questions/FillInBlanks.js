...
const FillInBlanks = props => {
  ...
  const [formData, setFormData] = useState(() => {
    ...
    domContent.each((index, item) => {
      ...
      var answerTemplate =
        "<div class='editView editablediv crtAnsDiv fbansw' type='text' id='RESPONSE_" +
        blankCount +
        "' >" +
        String.fromCharCode(65 + blankCount - 1) +
  ...
  const handleBlank = e => {
    ...
    var textEntryInteraction =
      '<button id="RESPONSE_$index" class="blankFIBButton">' +
      ...
      <div className="mb-1 d-flex align-items-center m-2 addfolder-container">
        <div className="flex-grow-1 d-flex ml-7 d-flex align-items-start">
          ...
          <div
            ...
            dangerouslySetInnerHTML={sanitizedData(getPrintModeFbCaption(text.current))}
          ></div>
        </div>
      </div>
    );
